...
skip_unload = 0
in_queue = False
# door_out_to_in_approach_queue = ["5.44927609813,2.67617526965,-0.913917053199,0.405900997623","3.61738583916,3.36497081915,-0.384114658816,0.923285399474","3.06435692002,3.94748137119,-0.387565199342,0.921842294679"]
door_out_to_in_approach_queue = ["0.377783249618,7.92581800226,-0.384135527805,0.923276717067","0.727836099911,8.8325540898,-0.609452595003,0.792822511312","1.14748866292,10.196517532,-0.806784839161,0.590845346347"]
door_in_to_out_approach_queue =  ["2.208109168,6.1860268899,0.89695345947,0.442124972767","2.8203728959,5.31996072077,0.908903521295,0.417006461553","3.59704008847,4.26534879618,0.89894729379,0.438056803381"]
drop_locations = ["1.55117261151,4.0696497805,-0.904846625857,0.425737693511","2.28223679285,3.65120902641,-0.896698598068,0.442641643118","3.21950617808,3.00702268077,-0.923767109947,0.382954731763"]
...
def get_token_for_going_in():
    ...
    while True:
        try:
            ...
            print('Error in get_token_for_going_in')
            continue
        break
...
def get_token_for_going_out():
    ...
    while True:
        try:
            ...
            print('Error in get_token_for_going_out')
            continue
        break
...
def get_token_for_loading_object(object_name):
    ...
    try:
        ...
        print('Error in get_token_for_loading_object')
...
def wait_for_gothrough_turn():
    while True:
        try:
            ...
            print('Error in wait_for_gothrough_turn')
            continue
        break
...
def wait_for_gothrough_turn_out():
    while True:
        try:
            ...
            print('Error in wait_for_gothrough_turn_out')
            continue
        break
...
def increment_ticket(action):
    ...
    while True:
        try:
            ...
            print('Error in increment_ticket')
            continue
        break

def update_door_reached():
    while True:
        try:
            response = requests.post('http://makeandroidapp.com/ar_demo/queue_manager/door_manager.php', data={'token':token_no,'robot':robot_no})
        except:
            print('Error in update_door_reached')
            continue
        break
...
def update_loaded():
    while True:
        try:
            response = requests.post('http://makeandroidapp.com/ar_demo/queue_manager/load_token_manager.php', data={'robot':robot_no})
        except:
...
def update_door_animation():
    while True:
        try:
            response = requests.post('http://makeandroidapp.com/ar_demo/queue_manager/load_token_manager.php', data={'reset':robot_no})
        except:
            print('Error in update_door_animation')
            continue
        break

def update_robot_reached():
    while True:
        try:
            response = requests.post('http://makeandroidapp.com/ar_demo/queue_manager/robots_reached.php', data={'robot':robot_no})
        except:
            print('Error in update_robot_reached')
            continue
        break
...
def ar_planner(task_to_be_done):
    ...
    #     f.close()
    # old_current_room_ar = current_room
    # trial_current_room = old_current_room_ar
    # iteration_count = 0
    # for goal in goal_list:
    #     task = goal[2]
    #     if(task == 3):
    #         object_name = goal[0]
    #         object_location_list_ar = location_list.find_object_loacation(object_name)
    #         object_location_ar = object_location_list_ar[0]
    #         print "Loading Object Task"
    #         print object_location_ar
    #         ar_plan.make_dummy_ar_plan(object_location_ar)
    #     elif(task == 4):
    #         print "Unloading Task"
    #         object_location_ar = drop_locations[0]
    #         ar_plan.make_ar_plan(object_location_ar)
    #     else :
    #         door_list = location_list.find_location(trial_current_room)
    ...
    #                 final_door_coordinate_ar = door_in_to_out_approach_queue[0]
    #             else:
    #                 final_door_coordinate_ar = door_out_to_in_approach_queue[0]
    #             new_location = door_locations[1]
    #
    #             if(task == 0):
    #                 if(approach_door_coordinate == door_out_to_in_approach_queue[0]):
    #                     final_door_coordinate = door_in_to_out_approach_queue[0]
    #                     # ar_plan.make_ar_plan(door_out_to_in_approach_queue[0])
    #                 else :
    #                     final_door_coordinate = door_out_to_in_approach_queue[0]
    #                     ar_plan.make_ar_plan(door_in_to_out_approach_queue[0])
    #             elif(task == 1):
    #                 print "Opening door."
    #                 # print final_door_coordinate
    #             elif(task == 2):
    #                 print "Going through door"
    #                 if(final_door_coordinate == "2.208109168,6.1860268899,-0.384135527805,0.923276717067"):
    #                     ar_plan.make_ar_plan(final_door_coordinate_ar)
    #                     trial_current_room = new_location
    #         else:
    #             print "DOnt know why it came here"
    ...
    #             if(new_location_name == "ta_1" and trial_current_room != "n09"):
    #                 location_to_go_ar  = door_out_to_in_approach_queue[0]
    #                 ar_plan.make_ar_plan(location_to_go_ar)
    #             else:
    #                 location_to_go_ar = location_coordinate[0]
    #             # ar_plan.make_ar_plan(location_to_go_ar)
    # print "Planning done"
    # ar_plan.upload_to_server()
    # print "Plan Uploaded to server."
    # print trial_current_room
    # trial_current_room = old_current_room_ar
                ...
                if(task == 3):
                    ...
                    if(location_name in door_list):
                        ...
                        if(task == 0):
                            ...
                            if(final_door_coordinate == "2.208109168,6.1860268899,-0.384135527805,0.923276717067"):
                                update_robot_reached()
                                while(not wait_for_gothrough_turn()):
                                    time.sleep(2)
                                print "My turn has arrived to go in"
                            if(final_door_coordinate == "0.377783249618,7.92581800226,0.89695345947,0.442124972767"):
                            ...
                            if(new_location == "ta_1" and current_room == "n09"):
                                token_no = -99
                        ...
                        # else:
                        #     "Changing room"
                        if(new_location_name == "ta_1" and current_room != "n09"):
...
def get_values(data):
        start_x = data.pose.pose.position.x
        start_y = data.pose.pose.position.y
        response = requests.post('http://makeandroidapp.com/ar_demo/queue_manager/update_position_blue_robot.php', data={'x':start_x,'y':start_y})


def callback_pose_upload(data):
        global previous_x
        global previous_y
        start_x = data.pose.pose.position.x
        start_y = data.pose.pose.position.y
        if(abs(previous_x-start_x)>1.0 or abs(previous_y-start_y)>1.0):
            previous_x = start_x
            previous_y = start_y
            response = requests.post('http://makeandroidapp.com/ar_demo/queue_manager/update_position_blue_robot.php', data={'x':start_x,'y':start_y})
...
def update_values():
    ...
    while(1):
        if(completed):
            return
        rospy.Subscriber ('/amcl_pose',PoseWithCovarianceStamped, callback_pose_upload)
        if(current_room !="ta_1"):
            clear_cost_map()
        #change_speed
        rospy.sleep(0.5)
        time.sleep(10)
...
#        rospy.spin()
        ...
        with open('entire_plan_robot_1.txt', 'w') as f:
            f.close()
        print "begin"
        # goto_location("5.44927609813,2.67617526965,-0.913917053199,0.405900997623",0)

        multitask_begin()
        # opendoor("5.44927609813,2.67617526965,-0.913917053199,0.405900997623","3.66259416799,0.694994267136,-0.913917053199,0.405900997623")
        # goto_location("-3.89822570943,-4.08039939243,0.86420337266,0.503142654406",0)
        # time.sleep(10)
        # goto_location("5.44927609813,2.67617526965,-0.913917053199,0.405900997623",0)
